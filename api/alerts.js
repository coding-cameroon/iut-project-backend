...
router.post('/', upload.array('photos', 5), [
  ...
  try {
    ...
    const mediaData = req.files ? req.files.map(file => ({
...
router.put('/:id', [
  ...
  body('status').optional().isIn(['PENDING', 'IN_PROGRESS', 'RESOLVED', 'CANCELLED']).withMessage('Statut invalide'),
  body('priority').optional().isIn(['LOW', 'MEDIUM', 'HIGH', 'CRITICAL']).withMessage('Priorité invalide')
], async (req, res) => {
  try {
    const errors = validationResult(req);
    ...
    if (io) {
      ...
      io.to(`user-${updatedAlert.userId}`).emit('alert-updated', updatedAlert);
    }
...
router.post('/:id/assign', [
  body('assignedTo').notEmpty().withMessage('Le gestionnaire assigné est requis'),
  ...
  try {
    ...
    const io = req.app.get('io');
    if (io) {
      io.to(`manager-${assignedTo}`).emit('alert-assigned', { alertId: id, assignment });
    }
