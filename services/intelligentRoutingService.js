...
class IntelligentRoutingService {
  ...
  static async routeAlert(alert, analysis) {
    ...
    try {
      ...
      const routingScores = await this.calculateRoutingScores(alert, analysis, emergencyServices);
      
      // Sélection des services les plus appropriés
      const selectedServices = this.selectOptimalServices(routingScores, analysis);
      
      // Attribution des services
      const assignments = await this.assignServicesToAlert(alert.id, selectedServices);
      
      // Envoi des notifications
      const notifications = await this.sendNotifications(alert, selectedServices, analysis);
      
      const processingTime = Date.now() - startTime;
      
      // Enregistrement des métriques
      await monitoringService.recordPerformance('intelligent_routing', processingTime, {
        alertId: alert.id,
        servicesSelected: selectedServices.length,
        confidence: analysis.classification.confidence
      });

      console.log(`Routage intelligent terminé pour l'alerte ${alert.id} en ${processingTime}ms`);
      ...
      console.error(`Erreur lors du routage intelligent de l'alerte ${alert.id}:`, error);
   ...
   */
  static async calculateServiceScore(alert, analysis, service, relevantServiceIds) {
    const factors = {
      typeMatch: 0,
      priorityMatch: 0,
      locationProximity: 0,
    ...
    factors.priorityMatch = this.calculatePriorityMatch(analysis.priority.level, service.type);
    
    // 3. Proximité géographique
    factors.locationProximity = await this.calculateLocationProximity(alert, service);
    ...
    factors.expertise = await this.calculateServiceExpertise(service, alert.type);
  ...
  static async calculateServiceExpertise(service, alertType) {
    try {
      ...
      const total = resolutionStats.reduce((sum, stat) => sum + stat._count.status, 0);
      const completed = resolutionStats.find(stat => stat.status === 'COMPLETED')?._count.status || 0;
  ...
  static calculateConfidence(factors) {
    const nonZeroFactors = Object.values(factors).filter(factor => factor > 0).length;
  ...
  static selectOptimalServices(routingScores, analysis) {
    ...
    const qualifiedServices = routingScores.filter(score => 
      score.score.total >= minScore && score.confidence >= 0.5
    );
  ...
  static async assignServicesToAlert(alertId, selectedServices) {
    ...
    for (const serviceScore of selectedServices) {
      try {
        ...
        console.error(`Erreur lors de l'assignation du service ${serviceScore.service.id}:`, error);
      }
    }
  ...
  static async sendNotifications(alert, selectedServices, analysis) {
    ...
    for (const serviceScore of selectedServices) {
      try {
        const notification = await AIService.sendToEmergencyServices(alert, analysis);
        notifications.push({
        ...
        console.error(`Erreur lors de la notification du service ${serviceScore.service.id}:`, error);
        notifications.push({
          serviceId: serviceScore.service.id,
          serviceName: serviceScore.service.name,
          success: false,
